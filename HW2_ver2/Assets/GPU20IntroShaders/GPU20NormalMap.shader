        ...
        Pass {
            ...
            v2f VertNormalMap(a2v input) {
     		...
     		float4 FragNormalMap(v2f input) : COLOR {
				...
				float2 nMap2XY = 2 * tex2D(_NormalMap2, input.nmap_uv).ag - 1;


				float nMapRecreatedZ = sqrt(1 - saturate(dot(nMapXY,nMapXY)));

				float nMap2RecreatedZ = sqrt(1 - saturate(dot(nMap2XY, nMap2XY)));
				...
				float3 newNormal2 = tWS * nMap2XY.x + btWS * nMap2XY.y + nWS * nMap2RecreatedZ;
				newNormal2 = normalize(newNormal2);

				normX = lerp(newNormal.x, newNormal2.x, 0.5*(1.0 + sin(_Rate*_Time.x)));
				normY = lerp(newNormal.y, newNormal2.y, 0.5*(1.0 + sin(_Rate*_Time.x)));
				normZ = lerp(newNormal.z, newNormal2.z, 0.5*(1.0 + sin(_Rate*_Time.x)));
				float3 norm = (normX,normY,normZ);
				...
				float3 diffAlmost = _LightColor0.rgb * max(0, dot(norm, lightDir)) * atten;
